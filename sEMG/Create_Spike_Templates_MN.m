% Create_Spike_Templates_MN || with decaying
% (per motoneuron)
...
for i_el = 1:2 % repeat for each electrode
    ...
    cond_fiberend = (velprop*t>=Lhalf); %IAP stops when it has run Lhalf (from endplat to end fiber)
    
    stop_index_fiber = Nt-sum(cond_fiberend,2)+1;
    t0 = t(stop_index_fiber)';
    t_aux = t-t0;
    num = ones(nf,Nt); num(:,cond_fiberend)= repmat(exp(-t_aux(cond_fiberend))/Tdecay,nf,1);
    
    %each row is a fiber, columns are instants of time
    z1 = velprop*(t)+z_fe; %the muap moves along the fiber
    z1(:,cond_fiberend) = repmat(Lhalf-bss+z_fe,1,Nt-stop_index_fiber+1); %we stop at the end of the fiber
    z2 = z1+bss; % separation between the sink and source: bss
    r1 = sqrt(pos_feT_2*alpha_cond + z1.^2); % distance to current source (I+)
    r2 = sqrt(pos_feT_2*alpha_cond + z2.^2); % distance to current sink (I-)
    %decaying when the IAP reaches the end of the fiber
    
    % MUAP MOVING AWAY FROM ELECTRODE
    z3 = -velprop*(t)+z_fe;
    
    z3(:,cond_fiberend) = repmat(-Lhalf+bss+z_fe,1,Nt-stop_index_fiber+1);
    %we stop at the end of the fiber
    z4 = z3-bss;
    r3 = sqrt(pos_feT_2*alpha_cond + z3.^2); % distance to current source (I+)
    r4 = sqrt(pos_feT_2*alpha_cond + z4.^2); % distance to current sink (I-)
    

    % MUAP
    phi_fib =  I.*kpot.*num.*( 1./r1-1./r2 - 1./r3 + 1./r4);
    potFib{i_el} = phi_fib;
...
%% DECAYING
% forward = ones(1,Nt); backward = ones(1,Nt);
% %exponential decay
% Tdecay = 0.06; L = 5;
...
%clear xc yc nf r_mn velprop zf pos_f Nt alpha_cond kpot pos_fel zef
%pos_ef2 r1 r2 z3 r3 r4 z1 z3
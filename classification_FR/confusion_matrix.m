...
test_accuracy = sum(predicted == labels)/length(labels)*100;
figure('visible','on');
imagesc(Cmat);colormap(flipud(jet));caxis([0,1])
...
title(sprintf('%s %.2f %s','Classification of EMG signals with',test_accuracy,'% accuracy'))
colorbar;
